...
The bootstrap initializes opam without changing shell startup files, creates an OCaml `4.14.2` switch beneath `.toolchain/`, and installs only the dependencies declared by the source package. Initialization uses `--no-opamrc` so host configuration cannot override the recorded setup. It uses `--disable-sandboxing` because the Ubuntu 24.04 hosted runner does not provide `bwrap` and PEEL's acquisition policy forbids `sudo`. As a result, opam package build scripts execute with the invoking user's ambient permissions; the opam repository and resolved package sources are therefore explicitly part of the trusted computing base. The flag grants no elevated privileges, and all installation destinations remain repository-local. System dependency installation is disabled; a missing host package makes acquisition fail. The bootstrap runs the source archive's `make install_bin` target and KaRaMeL's `minimal` target directly. KaRaMeL's source archive contains no nested Git metadata, so the build sets `GIT_CEILING_DIRECTORIES` to prevent discovery of PEEL's own `.git` directory and supplies the exact locked KaRaMeL revision through `GIT_REV`. Both locally built executables must report the locked provenance before they replace the corresponding executables in the extracted official distribution.
